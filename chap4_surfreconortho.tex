...
we then split the triangle $(v_i, v_j, v_k)$ to triangles $(v_i, v_j, v_c)$,
...
repeat the triangle splitting step; otherwise, the refinement process is complete.
...
\begin{equation}
\label{eq:surfsmooth}
    p^\prime_i = p_i + \tau N_i + \epsilon T_i.
\end{equation}
In the above equation, $N_i$ is the unit normal vector at $v_i$. We compute $N_i$
as the normalized weighted sum of of the normals of the incident triangles of $v_i$, 
with weights equal to the areas of the triangles. We set the step size
$\tau = A^2/150$ as suggested in~\cite{YB02}, where $A$ is the minimum area 
among all the incident triangles. To improve the stability of the surface
evolution through making the triangle sizes as equal as possible, we then 
move the vertex along a tangential vector $T_i$. More precisely, we compute it
as $T_i = \delta_i + N_i$, where $\delta_i$ is the vector pointing from $v_i$
to the centroid of all its neighbor vertices. The step size $\epsilon$ 
is set to $0.5$ in Eq.~\ref{eq:surfsmooth}. Similarly, the smoothing is only 
