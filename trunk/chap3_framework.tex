...
\label{eq:planeobjnoZ0}
    f(A,B,C,D)=\sum\limits_{i \in HC} {\frac {(Ax_i+By_i+Cz_i+D)^2}{A^2+B^2+C^2}},
\end{equation}
subject to the constraint $A^2+B^2+C^2=1$, where $i \in HC$  means
$p_i=(x_i,y_i,z_i)$ is a point on the handle curve.
...
\label{eq:planeobjnew}
    f(A,B,C) &=& \sum\limits_{i \in HC} {\frac {(A(x_i-x_c)+B(y_i-y_c)+C(z_i-z_c))^2}{A^2+B^2+C^2}}\nonumber\\
    &=& (P^TM^T)(MP)/(P^TP)\nonumber\\
    &=& P^T (M^TM) P/(P^TP),
\end{eqnarray}
where $P^{\rm T} = [\begin{array}{*{20}c} A & B & C \end{array}]$
...
of $M=USV^{\rm T}$ and letting $P$ equal to the third column 
of $U$. To this end, the parameters of the optimal plane in
